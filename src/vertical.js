...
(() => {
  ...
  function actionButton(name, text, label, action) {
    const el = button('', label, action);
    el.append(icon(name), document.createTextNode(text));
    return el;
  }
  ...
  async function open() {
    ...
    const version = chrome.runtime.getManifest?.().version || '3.3.2';
    ...
    root.innerHTML = `<style>
      ...
      * { box-sizing: border-box; border-radius: 0; }
      ...
      .column header { position: relative; z-index: 1; padding: 9px; display: flex; align-items: center; gap: 4px; background: var(--xh-bg); border-bottom: 1px solid var(--xh-border); min-height: 48px; }
      ...
      .column header button, .column header a, .column summary { display: inline-flex; align-items: center; justify-content: center; gap: 5px; height: 28px; padding: 4px 7px; font: 12px var(--xh-sans); letter-spacing: 0; }
      .column svg { width: 14px; height: 14px; flex: none; fill: none; stroke: currentColor; stroke-width: 1.7; stroke-linecap: square; stroke-linejoin: miter; }
      .column summary { list-style: none; cursor: pointer; border: 1px solid var(--xh-border); color: var(--xh-secondary); user-select: none; }
      .column summary::-webkit-details-marker { display: none; }
      .column summary:hover { background: var(--xh-text); color: var(--xh-bg); }
      .column details[open] summary, .column button[aria-pressed="true"] { border-color: var(--xh-highlight); color: var(--xh-highlight); background: var(--xh-surface); }
      .column-menu { position: absolute; right: 9px; top: calc(100% - 1px); width: 188px; padding: 5px; display: grid; gap: 2px; border: 1px solid var(--xh-border); border-top-color: var(--xh-highlight); background: var(--xh-surface); box-shadow: 0 6px 16px #0009; }
      .column .column-menu button, .column .column-menu a { justify-content: flex-start; height: 32px; padding: 6px 8px; width: 100%; border: 1px solid transparent; color: var(--xh-secondary); text-decoration: none; }
      .column-menu .separator { height: 1px; margin: 3px 0; background: var(--xh-border); }
      .column .column-menu button:hover, .column .column-menu a:hover { color: var(--xh-bg); }
      ...
      <div class="toolbar"><span class="brand">X HUB <span class="mode-label">VERTICAL</span></span><span class="tag">여러 타임라인, 한 화면</span><span class="spacer"></span><button class="primary" id="add">+ 열 추가</button><button id="exit">기본 화면으로</button></div>
      <div class="deck"></div>
    ...
    root.addEventListener('pointerdown', event => {
      root.querySelectorAll('details[open]').forEach(menu => { if (!menu.contains(event.target)) menu.open = false; });
    });
  ...
  function resize() {
    ...
    const count = Math.max(1, Math.min(config.columns.length, 5, Math.floor((host.clientWidth - 10) / (MIN_COLUMN_WIDTH + 10))));
    deck.style.setProperty('--column-width', `${Math.max(MIN_COLUMN_WIDTH, (host.clientWidth - 10) / count - 10)}px`);
  }
  function save() {
  ...
  function createColumn(column) {
    ...
    function load() {
      ...
      frame.src = url.href;
      item.timer = setTimeout(() => { status.textContent = '로드가 지연되거나 X가 임베딩을 차단했습니다. 새로고침하거나 설정 → 새 탭에서 열기를 이용하세요.'; status.hidden = false; }, 25000);
    }
    ...
    const toggle = document.createElement('summary'); toggle.append(icon('settings'), document.createTextNode('설정')); toggle.setAttribute('aria-label', `${column.title} 열 설정`);
    const options = document.createElement('div'); options.className = 'column-menu';
    const menuAction = action => () => { menu.open = false; toggle.focus(); action(); };
    menu.onkeydown = event => { if (event.key === 'Escape') { event.preventDefault(); menu.open = false; toggle.focus(); } };
    menu.onfocusout = () => queueMicrotask(() => { if (!menu.contains(root?.activeElement)) menu.open = false; });
    item.left = actionButton('left', '왼쪽으로 이동', `${column.title} 왼쪽으로 이동`, menuAction(() => move(-1)));
    item.right = actionButton('right', '오른쪽으로 이동', `${column.title} 오른쪽으로 이동`, menuAction(() => move(1)));
    item.expand = actionButton('expand', '확대', `${column.title} 열 확대`, () => {
      const on = !node.classList.contains('focus');
      frames.forEach(other => {
        const active = other === item && on;
        other.node.classList.toggle('focus', active);
        other.expand.setAttribute('aria-pressed', String(active));
        const label = `${other.frame.title.replace(/ 타임라인$/, '')} 열 ${active ? '복귀' : '확대'}`;
        other.expand.title = label; other.expand.setAttribute('aria-label', label);
        other.expand.replaceChildren(icon(active ? 'collapse' : 'expand'), document.createTextNode(active ? '복귀' : '확대'));
      });
      deck.classList.toggle('focused', on);
    });
    item.expand.setAttribute('aria-pressed', 'false');
    const external = document.createElement('a'); external.append(icon('external'), document.createTextNode('새 탭에서 열기')); external.href = new URL(column.path, location.origin).href; external.target = '_blank'; external.rel = 'noopener noreferrer'; external.onclick = menuAction(() => {});
    const separator = document.createElement('div'); separator.className = 'separator';
    options.append(item.left, item.right, external, separator, actionButton('delete', '열 삭제', `${column.title} 열 삭제`, menuAction(() => {
    ...
    header.append(title, actionButton('refresh', '새로고침', `${column.title} 새로고침`, load), item.expand, menu);
  ...
  function prepareColumn() {
    ...
    css.textContent = `
      ...
      html[data-xhub-column],html[data-xhub-column] *{scrollbar-width:none!important}
      html[data-xhub-column]::-webkit-scrollbar,html[data-xhub-column] *::-webkit-scrollbar{display:none!important;width:0!important;height:0!important}
