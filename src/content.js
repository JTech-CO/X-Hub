(() => {
  const isVerticalColumn = globalThis.XHubVertical?.isColumn === true;
  if (window !== window.top && !isVerticalColumn) return;
  // --- Utilities ---
  ...
  // --- Scope Detection ---
  let _raffleScopeIsPrimary = true;
  ...
  function detectRetweetsScope() {
    ...
    for (const sel of labelSelectors) {
      ...
      if (el) { _raffleScopeIsPrimary = false; return el; }
    }
    ...
    if (regions.length) {
      ...
      if ($$('[data-testid="UserCell"]', regions[0]).length > 0) { _raffleScopeIsPrimary = false; return regions[0]; }
    }

    _raffleScopeIsPrimary = true;
    return primary;
  }
  let RETWEETS_SCOPE = detectRetweetsScope();
  ...
  function getScrollTarget(scopeEl) {
    ...
    const canScroll = (n) => {
      ...
      return n.scrollHeight > n.clientHeight + 20 && ['auto', 'scroll', 'overlay'].includes(oy);
    };
    let p = scopeEl;
  ...
  if (isVerticalColumn) host.style.display = 'none';
  ...
  styleEl.textContent = `
    ...
    .card { overflow-x: hidden; overflow-y: auto; }
    .card-body {
      transition: opacity 0.22s ease;
    }
    .card-body.xhub-hidden {
      opacity: 0;
      pointer-events: none;
    }
    @keyframes xhub-slide-down {
      from { opacity: 0; transform: translateY(-8px); }
      to   { opacity: 1; transform: translateY(0); }
    }
    .card-body.xhub-slide-in {
      animation: xhub-slide-down 0.22s ease forwards;
    }
  `;
  ...
  miniWrap.innerHTML = '<div class="xhub-mini-dot"></div><span class="xhub-mini-logo">X HUB</span><button class="btn orange" id="vertical-shortcut" title="Vertical 멀티 타임라인">Vertical</button>';
  shadow.append(styleEl, miniWrap, wrap);

  // --- Collapse / Expand ---
  let isPanelCollapsed = false;
  let _expandPending = false;
  ...
  function expandPanel(switchToMode) {
    // Prevent duplicate calls while animation is in progress
    if (_expandPending) return;
    _expandPending = true;
    isPanelCollapsed = false;

    // Step 1: keep mini visible (짧은 바), start widening host → 긴 바
    // Card stays hidden while mini stretches across
    wrap.style.display = 'none';
    host.style.width = '440px';

    // Step 2: after width transition completes (긴 바 reached),
    //         hide mini and slide the full panel in
    const onWidthDone = () => {
      ...
      wrap.style.display = '';
      const cb = wrap.querySelector('.card-body');
      if (cb) {
        cb.classList.remove('xhub-hidden', 'xhub-slide-in');
        // Force reflow so animation triggers fresh
        void cb.offsetHeight;
        cb.classList.add('xhub-slide-in');
        setTimeout(() => cb.classList.remove('xhub-slide-in'), 280);
      }
    };

    // Listen for host's width transition end, fallback to 300ms
    let fired = false;
    const onTransEnd = (e) => {
    ...
    setTimeout(() => { if (!fired) { fired = true; host.removeEventListener('transitionend', onTransEnd); onWidthDone(); } }, 300);
  }

  miniWrap.onclick = () => expandPanel();
  function openVertical() {
    raffleRunning = false; followRunning = false;
    globalThis.XHubVertical?.open();
    host.style.display = 'none';
  }
  shadow.getElementById('vertical-shortcut').onclick = event => { event.stopPropagation(); openVertical(); };
  window.addEventListener('xhub-vertical-close', () => { host.style.display = ''; collapsePanel(); });
  chrome.runtime.onMessage.addListener(msg => {
    if (msg.action !== 'togglePanel' || isVerticalColumn) return;
    globalThis.XHubVertical?.close();
    host.style.display = '';
    if (isPanelCollapsed) expandPanel(); else collapsePanel();
  });
  ...
  const MODES = ['raffle', 'follow', 'filter', 'vertical'];
  const MODE_LABELS = { raffle: 'Raffle', follow: 'Follow', filter: 'Filter', vertical: 'Vertical' };
  ...
  function renderModeBody() {
    switch (currentMode) {
      ...
      case 'vertical':
        modeBody.innerHTML = '<div class="col"><p>홈·검색·알림·리스트를 여러 열로 동시에 확인하세요.</p><button id="vertical-open" class="btn accent">Vertical</button><span class="footnote">X HUB의 멀티 타임라인 기능입니다. 각 열에서 X의 기본 기능을 사용할 수 있습니다.</span></div>';
        shadow.getElementById('vertical-open').onclick = openVertical;
        break;
    }
  }
  ...
  const RAFFLE_MAX_STABLE = 8;
  const RAFFLE_PAUSE = 300;
  const RAFFLE_MAX_TICKS = 500;
  let RAFFLE_SCROLLER = getScrollTarget(RETWEETS_SCOPE);
  ...
  function raffleParseRT() {
    ...
    for (const c of cells) {
      ...
      try {
        let handle = '';
        const links = $$('a[href^="/"], a[href^="https://x.com/"], a[href^="https://twitter.com/"]', c);
        for (const a of links) {
          const href = a.getAttribute('href') || '';
          if (/^https?:\/\/(x|twitter)\.com\//.test(href)) {
            const seg = href.split(/\.com\//)[1]?.split('?')[0];
            if (seg && !seg.startsWith('i/') && !seg.includes('/status/')) { handle = seg.replace(/\/+$/, '').toLowerCase(); break; }
    ...
    modeBody.innerHTML = `
      <div class="col">
        <div class="row" style="flex-wrap:wrap">
          ...
          <button id="r-fo" class="btn secondary" title="Show only users who follow me" ${raffleExtracted && raffleUsers.length > 0 ? '' : 'disabled'}>Follower Only</button>
        </div>
        <div class="row" style="flex-wrap:wrap">
      ...
      <span class="footnote">Scope: ${_raffleScopeIsPrimary ? 'PRIMARY' : 'DETECTED'}</span>
    `;
    wireRaffle();
    raffleRenderList();
  }
  ...
  async function raffleAutoScroll() {
    const runId = ++raffleRunId, runURL = location.href;
    const active = () => raffleRunning && runId === raffleRunId && location.href === runURL;
    RETWEETS_SCOPE = detectRetweetsScope();
    RAFFLE_SCROLLER = getScrollTarget(RETWEETS_SCOPE);
    ...
    for (let tick = 0; active() && tick < RAFFLE_MAX_TICKS; tick++) {
      const added = raffleParseRT();
      if (deltaEl) deltaEl.textContent = String(added);
      raffleRenderList();
      if (raffleUsers.length === raffleLastCount) {
        raffleStableTicks++;
        if (raffleStableTicks >= RAFFLE_MAX_STABLE) {
          // 가상스크롤 로딩 여유를 위해 한 번 더 스크롤 후 재확인
          doScroll(RAFFLE_SCROLLER);
          await sleep(800);
          if (!active()) break;
          const retryAdded = raffleParseRT();
          if (retryAdded > 0) {
            if (deltaEl) deltaEl.textContent = String(retryAdded);
    ...
    if (runId !== raffleRunId || location.href !== runURL) return;
  ...
  function raffleToCSV(rows) {
    ...
    const escCSV = csvCell;
    return [header.join(','), ...rows.map(r => [r.handle, r.nickname, r.description].map(escCSV).join(','))].join('\n');
  }

  function copyText(txt, statusId) {
    const statusEl = shadow.getElementById(statusId);
    navigator.clipboard.writeText(txt).then(() => report('copied'), () => report('copy failed'));
    function report(message) {
      if (!statusEl) return;
      statusEl.textContent = message;
      setTimeout(() => { if (statusEl.isConnected) statusEl.textContent = 'idle'; }, 1500);
    }
  }
  const raffleCopyText = txt => copyText(txt, 'r-status');

  // --- MODE 2: Follow ---
  let followRunning = false;
  let followUsers = [];
  let followSeen = new Set();
  let _lastFollowPageType = null;
  ...
  function panelAutoState() {
    if (isVerticalColumn) return;
    const pg = getPageType();
    const rl = isRafflePage();
    if (rl) {
      // Always expand with animation before switching mode
      if (isPanelCollapsed) {
        expandPanel('raffle');
      } else if (currentMode !== 'raffle') {
        raffleRunning = false; followRunning = false;
        currentMode = 'raffle';
      ...
      if (isPanelCollapsed) {
        ...
        renderModeBody();
      } else if (pg !== _lastFollowPageType) {
        // followers ↔ following 전환 시 이전 데이터 초기화 후 재렌더
        followRunning = false;
  ...
  function parseFollowCell(cell) {
    ...
    for (const btn of buttons) {
      ...
      if (testid.toLowerCase().includes('unfollow') ||
        btnText.includes('팔로잉') || btnText.includes('Following')) {
        isMyFollowing = true;
        break;
      }
    }
  ...
  async function followAutoScroll() {
    const runId = ++followRunId, runURL = location.href;
    const active = () => followRunning && runId === followRunId && location.href === runURL;
    ...
    for (let tick = 0; active() && tick < FOLLOW_MAX_TICKS; tick++) {
    ...
    if (runId !== followRunId || location.href !== runURL) return;
    if (statusEl) {
  ...
  const followCopyText = txt => copyText(txt, 'f-status');
  ...
  const filterSettings = value => ({ ...FDEF, ...value, filter: { ...FDEF.filter, ...value?.filter } });
  let fS = filterSettings();
  let fFollowSet = new Set();
  let fWlSet    = new Set();
  const fBc     = new Map();
  let fObs = null, fPend = [], fRafQ = false;
  let _collectDomObs = null;

  // Badge cache with fixed-size LRU eviction
  const FBC_MAX = 10000;
  function fBcSet(key, val) {
    fBc.delete(key);
    if (fBc.size >= FBC_MAX) fBc.delete(fBc.keys().next().value);
    fBc.set(key, val);
  }
  ...
  function fParseBadge(u) {
    ...
    if (!handle || typeof u.is_blue_verified !== 'boolean') return null;
    return { id: handle.toLowerCase(), prem: u.is_blue_verified && !['Business', 'Government'].includes(u.verified_type) };
  }
  ...
  function fAnyBadge(el) {
    el = el.querySelector('[data-testid="User-Name"], [data-testid="UserName"]') || el;
  ...
  function fChkPrem(uid, el) {
    return fBc.get(uid.toLowerCase()) ?? fSvgBadge(el.querySelector('[data-testid="User-Name"], [data-testid="UserName"]') || el);
  }
  ...
  const filterDisplays = new WeakMap();
  function filterHide(el, attribute = F_AH) {
    if (el.hasAttribute(attribute)) return;
    filterDisplays.set(el, [el.style.getPropertyValue('display'), el.style.getPropertyPriority('display')]);
    el.setAttribute(attribute, '1'); el.style.setProperty('display', 'none', 'important');
  }
  function filterShow(el) {
    const [value, priority] = filterDisplays.get(el) || ['', ''];
    if (value) el.style.setProperty('display', value, priority); else el.style.removeProperty('display');
    el.removeAttribute(F_AH); el.removeAttribute('data-xrr-qh'); filterDisplays.delete(el);
  }
  function filterRestoreTweet(tweet) {
    if (tweet.hasAttribute(F_AH)) filterShow(tweet);
    tweet.querySelectorAll('[data-xrr-qh]').forEach(filterShow);
  }
  function filterShowAll() {
    document.querySelectorAll(`[${F_AH}], [data-xrr-qh]`).forEach(el => { filterShow(el); el.removeAttribute('data-xrr-qh'); });
  }
  function fPageOk() {
    const p = location.pathname;
  ...
  function filterSched() {
    ...
    requestAnimationFrame(() => {
      const b = new Set(fPend.splice(0));
      for (const t of b) { if (t.isConnected) { filterRestoreTweet(t); filterProc(t); } }
      fRafQ = false;
    });
  }
  ...
  function filterStartObs() {
    filterStopObs();
    if (fS.mode === 'all') return;
    fObs = new MutationObserver(ms => {
      for (const m of ms) {
        const tweet = m.target.closest?.('article[data-testid="tweet"]');
        if (tweet) fPend.push(tweet);
    ...
    fObs.observe(document.body, { childList: true, subtree: true });
  }
  ...
  (() => {
    ...
    setInterval(() => {
      ...
      raffleRunning = false; followRunning = false;
      raffleUsers = []; raffleSeen.clear();
      followUsers = []; followSeen.clear();
      filterOnNav(); panelAutoState();
    }, 500);
  })();
  function filterOnNav() {
    filterShowAll();
    filterStopObs();
    if (_collectDomObs) { _collectDomObs.disconnect(); _collectDomObs = null; }
    requestAnimationFrame(() => { filterCollectDOM(); filterRunCollect(); filterStartObs(); filterReprocess(); });
  }
  ...
  function filterCollectDOM() {
    ...
    _collectDomObs = new MutationObserver(scrape);
    _collectDomObs.observe(document.body, { childList: true, subtree: true });
    setTimeout(scrape, 2000);
  }
  async function filterDetectAcct(retry = 0) {
    if (isVerticalColumn) return;
    const el = document.querySelector('a[data-testid="AppTabBar_Profile_Link"]');
    if (!el) {
      if (retry < 3) setTimeout(() => filterDetectAcct(retry + 1), 2000);
      return;
    }
    ...
    if (st[FSK.CU] !== handle) {
      ...
      await chrome.storage.local.set({ [FSK.CU]: handle, [FSK.FL]: follows, [FSK.WL]: [], [FSK.FRC]: 0, [FSK.FGC]: 0 });
  ...
  async function filterRunCollect() {
    if (isVerticalColumn) return;
    const owned = await chrome.runtime.sendMessage({ action: 'isCollectTab' }).catch(() => false);
    if (!owned) return;
    const path = location.pathname;
    ...
    const tick = async () => {
      const active = (await chrome.storage.local.get(jobKey))[jobKey];
      if (location.pathname !== path || active?.state !== 'running' || active.handle !== job.handle) return;
      scrape();
      ...
      if (stable >= 3) {
        await chrome.runtime.sendMessage({ action: 'finishCollect', handle: job.handle, isFollowers: isFr, handles: [...coll] });
  ...
  function filterListenMsg() {
    window.addEventListener('message', e => {
      ...
      if (e.data?.type === 'XFP_BADGE_DATA' && Array.isArray(e.data.users)) {
        for (const u of e.data.users) {
          const info = fParseBadge(u);
          if (info) fBcSet(info.id, info.prem);
        }
        filterShowAll(); filterReprocess();
      }
    });
  }
  function filterListenSettings() {
    chrome.storage.onChanged.addListener((ch, area) => {
      ...
      if (ch[FSK.S]) {
        fS = filterSettings(ch[FSK.S].newValue);
        filterShowAll(); filterStartObs(); filterReprocess();
      ...
      if (ch[FSK.FL] || ch[FSK.WL]) { filterShowAll(); filterReprocess(); }
    });
  }
  ...
  function filterSyncPoller() {
    ...
    _filterSyncPollerTimer = setInterval(async () => {
      ...
      if (st[FSK.CJ]?.state === 'running' || st[FSK.CJ2]?.state === 'running') {
        filterRenderSyncSection();
      } else if (st[FSK.CJ]?.state === 'done' && st[FSK.CJ2]?.state === 'done') {
        clearInterval(_filterSyncPollerTimer);
      }
    }, 1500);
  }
  ...
  function wireFilterMode() {
    ...
    const setFMode = async (mode) => {
      ...
      await saveF();
      renderFilterMode();
      if (fS.mode !== 'all') requestAnimationFrame(() => filterReprocess());
    };
